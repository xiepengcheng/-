...
bool dataserver::master_init(std::map<std::string, std::string>& para)//��Ҫ������ʼ��
{
...
bool dataserver::work_init()
{
	for(int i=0;i<thread_num;++i)
	{
		...
		eventfds.push_back(fds[1]);//�����̳߳�ʼ��
		event.push_back(ev);
		
		auto one_worker=std::make_unique<hodis::workthread>(fds[0],i, (*worker_item_aq)[i], (*worker_item_aq_condition)[i] );//�����̲߳���ʼ��
...
bool register_worker(int fd)//ע���߳��¼�
{
	Item item;
